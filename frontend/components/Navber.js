...
export default function Navber() {
  ...
  useEffect(() => {
    fetchUserData(); // Refresh user data on route change
  }, []);
    ...
    <>
      <nav className="fixed w-full top-0 z-50 bg-black/60 backdrop-blur shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between relative">
          ...
          <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-12 h-12 relative mt-4">
            <Image
      ...
      {/* Sidebar for mobile */}
      <div
        className={`fixed top-0 left-0 z-40 w-64 h-full bg-black text-white transform transition-transform duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <div className="p-6">
